...
# system.lio_pose_topic），支持隔离话题（如 /m20_slam/pose）；读取失败时
# 回退到 M20 默认名。
_DEFAULT_CORE_READY_TOPICS = ["/IMU", "/LIDAR/POINTS", "/lio_pose"]


def _ready_topics_from_config(config_path):
    """从 config yaml 读取门禁需要的传感器/LIO 输出话题名。"""
    ...
    try:
        ...
        topics[2] = str(sys_cfg.get("lio_pose_topic", topics[2]))
    except Exception:
        pass
    return topics
