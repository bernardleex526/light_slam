...
#include "m20_navigation/common/math_utils.hpp"
#include "m20_navigation/common/native_navigation.hpp"
#include "m20_navigation/common/params.hpp"
#include "m20_navigation/common/types.hpp"
#include "m20_navigation/control/local_controller_node.hpp"
#include "m20_navigation/ros/native_navigation_bridge.hpp"
#include "m20_navigation/terrain/traversability_map.hpp"
#include "m20_navigation/planning/global_planner_node.hpp"
...
#define LOAD_STRING(name, field) \
  ...
  bool acceptGoal(double x, double y, double yaw, const std::string& frame_id) {
    ...
    // P1-5 修复：新目标到达前先停止旧规划，防止旧 plan 在规划期间继续执行
    {
      std::lock_guard<std::mutex> lock(state_mutex_);
      if (goal_active_) {
        goal_active_ = false;
  ...
  void controlLoop() {
    ...
    if (distance <= controller_params_.stop_distance &&
      ...
      // P1-4 修复：到达后停止控制循环，防止持续发零命令与 kGoalReached
      {
        std::lock_guard<std::mutex> lock(state_mutex_);
        goal_active_ = false;
      }
